...
class Sum(MathFunction):
    ...
    def derivate(self, var: V):
        return self.a.derivate(var) + self.b.derivate(var)
...
class Prod(MathFunction):
    ...
    def derivate(self, var: V):
        ...
        return self.a.derivate(var) * self.b + self.a.derivate(var) * self.b.derivate(var)
...
class Frac(MathFunction):
    ...
    def derivate(self, var: V):
        """Quot rule: d/dx(f(x)/g(x)) = (f'(x)g(x) - f(x)g'(x)) / g(x)^2 """
        return (self.a.derivate(var) * self.b - self.a * self.b.derivate(var)) / self.b ** C(2)
...
class Pow(MathFunction):
    ...
    def derivate(self, var: V):
        ...
        if isinstance(self.b, Constant):
            ...
            return self.b * self.a ** C(self.b.value - 1)
            # return Prod(prod1, self.a.derivate(var))
        else:
            return self * (self.b.derivate(var) * Ln(self.a) + self.b * self.a.derivate(var) / self.a)
...
class Ln(MathFunction):
    ...
    def derivate(self, var):
        ...
        return self.a.derivate(var) / self.a
...
class Exp(MathFunction):
    ...
    def derivate(self, var):
        ...
        return self * self.a.derivate(var)
...
class Sin(MathFunction):
    ...
    def derivate(self, var):
        ...
        return Cos(self.a) * self.a.derivate(var)
...
class Cos(MathFunction):
    ...
    def derivate(self, var):
        ...
        return C(-1) * Sin(self.a) * self.a.derivate(var)
...
class Tan(MathFunction):
    ...
    def derivate(self, var):
        ...
        return self.a.derivate(var) / (Cos(self.a) ** C(2))
...
class Arcsin(MathFunction):
    ...
    def derivate(self, var):
        """Derivative of cos(x): d/dxsin(a(x)) = cos(a(x))*a'(x)"""
        return self.a.derivate(var) / Sqrt(C(1) - self.a ** C(2))
...
class Arccos(MathFunction):
    ...
    def derivate(self, var):
        """Derivative of cos(x): d/dxsin(a(x)) = cos(a(x))*a'(x)"""
        return C(-1) / (Sqrt(C(1) - self.a ** 2)) * self.a.derivate(var)
        # Frac(Prod(-1, self.a.derivate(var)), Sqrt(Sub(1, Pow(self.a, 2))))
...
class Arctan(MathFunction):
    ...
    def derivate(self, var):
        """Derivative of arctan(x): d/dx arctan(a(x)) = a'(x)/(1+a(x)^2)"""
        return self.a.derivate(var) / (C(1) + self.a ** C(2))
        # return Frac(1, Sum(1, Pow(self.a, 2))) falsch?
    ...
    def __repr__(self):
        return f"arctan({self.a})"

    def latex(self):
        return r"\arctan("+str(self.a.latex())+")"
...
def gauss_error(f: MathFunction, vars, uncerts):
    """
    :param f:       MathFunction object
    :param vars:    iterable with Variable objects
    :param uncerts: iterable with uncertainty values for each variable
    :return:        MathFunction

    #TODO: Translate: Gibt die gaußsche Fehlerfortpflanzung von f aus
    """
    sum = C(0)
    for i in range(len(vars)):
        sum += (f.d(vars[i]) * C(uncerts[i])) ** 2
    return Sqrt(sum.simplify())
...
"""
f = x ** 2 + Cos(y) / Arctan(z)
u = [2, 5, 6]
gf = gauss_error(f, [x, y, z], u)
print(gf)
print(gf.calc({x: 5, y: 42, z: 30}))
"""